...
public class File extends FsEntity {
    ...
    public File(int id, String name, String data) {
        super(id);
        this.data = data;
        this.size = (int) Math.ceil(data.length()/(float) DiskMemory.BLOCK_SIZE);
        HardLink link = new HardLink(Driver.id++,this, name);
        links.add(link);
        DiskMemory.blockFileTable[DiskMemory.getStartIndex(1)] = link;
    }
    ...
    public String toString() {
        return "lab4_5.File{" + "id='" + id + '\'' + ", size=" + size + ", descriptor=" + descriptor + ", data='" + data + '\'' + '}';
    }
