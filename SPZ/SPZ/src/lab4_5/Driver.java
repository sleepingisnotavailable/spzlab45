...
import static lab4_5.DiskMemory.*;
...
public class Driver {
    ...
    static void stat(String id) {
        Optional<File> optionalFile = getFilesFromDiskMemory()
                .stream()
                .filter(file -> Integer.parseInt(id) == file.descriptor)
                .findFirst();
        if (optionalFile.isPresent()) System.out.format("lab4_5.File with descriptor %s:\n%s\n", id, optionalFile.get());
        else System.out.format("lab4_5.File with descriptor %s is not found!\n", id);
    }
    ...
    static void create(String filename, String data) {
        if (filename.startsWith("/")){
            ...
            if (dir != null) {
                create(split[split.length-1], data, dir);
            } else System.out.println("No such directory!");
        } else create(filename, data, currentDir);
        System.out.println("lab4_5.File is created!");
    }

    private static void create(String filename, String data, Directory dir){
        if (filename.length() > MAX_FILE_LENGTH) {
            System.out.println("lab4_5.File name length too long!");
            return;
        }
        if (dir.hasFile(filename)) {
            System.out.format("lab4_5.File with name '%s' is exists!\n", filename);
            return;
        }
    ...
    static void open(String fileName) {
        if (currentDir.hasLink(fileName)){
            String[] split = getSymLinkByNameFromDiskMemory(fileName).pathName.split("/");
            if (limitLinkJump-- != 0) {
    ...
    static void close(String fd) {
        getFilesFromDiskMemory()
                ...
                .ifPresent(file -> {
                    ...
                    descriptor--;
                    System.out.format("lab4_5.File '%s' is closed\n", file.id);
                });
    }
    ...
    static void read(String fd, String shift) {
        getFilesFromDiskMemory()
                ...
                .ifPresent(file -> {
                    ...
                    else System.out.format("lab4_5.File data with shift %s: %s\n", shift,
        ...
        if (fileStream.count() > 1) {
            ...
            fileStream
                    ...
                    .ifPresent(file -> {
                        ...
                        System.out.format("lab4_5.Link %s to file '%s' is created!\n", linkName, fileName);
                    });
        }
    }
    ...
    static void unlink(String linkName) {
        currentDir.dirElements
                ...
                .ifPresent(file -> {
                    ...
                    System.out.format("lab4_5.Link '%s' is deleted!\n", linkName);
                    if (file.links.isEmpty()) {
                        ...
                        System.out.format("lab4_5.File '%s' is deleted!\n", file.id);
                    }
                });
    }
    ...
    static void truncate(String fileName, String newSize) {
        ...
        if (fileName.startsWith("/")) {
            ...
            if (isSymLink(split[split.length-1]) && limitLinkJump-- != 0)
                truncate(getSymLinkByNameFromDiskMemory(split[split.length-1]).pathName, newSize);
            ...
            if (blockFileTable[i] != null && blockFileTable[i] instanceof FilePart fp) {
                ...
                if (file.hasLink(fileName)) {
                    if (newSizeInt < file.size) {
                        ...
                        System.out.format("lab4_5.File size is decreased to %d for file '%s'\n", newSizeInt, fileName);
                        ...
                        System.out.format("lab4_5.File size is increased to %d for file '%s'\n", newSizeInt, fileName);
                    }
    ...
    static void mkDir(String dirName) {
        if (!currentDir.hasElement(dirName)) {
            ...
            System.out.format("lab4_5.Directory '%s' is created!\n", dirName);
        } else System.out.format("lab4_5.Directory with name '%s' exists\n", dirName);
    }
    ...
    static void rmDir(String dirName) {
        if (currentDir.hasDir(dirName)) {
            ...
            if (currentDir.getDirByName(dirName).isEmpty()) {
                ...
                System.out.format("lab4_5.Directory '%s' is removed!\n", dirName);
            } else System.out.println("lab4_5.Directory is not empty!");
        } else System.out.format("lab4_5.Directory with name '%s' does not exist\n", dirName);
    }
    ...
    static void cd(String dirName) {
        if ("..".equals(dirName)) {
            ...
            if (dirName.startsWith("/")) {
                ...
                } else System.out.println("lab4_5.Directory does not exist!");
            } else System.out.println("lab4_5.Directory does not exist!");
        }
    }
    ...
    static void symLink(String fileOrDirName, String linkName) {
        ...
        } else System.out.println("lab4_5.Link, dir or file with this name exists!");
    }
    ...
    private static Set<File> getFilesFromDiskMemory() {
    ...
    private static SymLink getSymLinkByNameFromDiskMemory(String name){
        return Arrays.stream(blockFileTable)
    ...
    private static File getFileByNameFromDiskMemory(String name){
        return Arrays.stream(blockFileTable)
